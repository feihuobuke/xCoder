...
using System.Linq;
using xCoder.Bean;
using xCoder.Logic.AbsBase;

namespace xCoder.Logic.Data.MsSql
{
    internal class MsSqlReader : AbsReader
    {
            ...
            using (Command)
            {
                ...
                try
                {
                    ...
                    using (var reader = Command.ExecuteReader())
                    {
                        ...
                        {
                            var tables = data.Tables[0].Rows.OfType<DataRow>().GroupBy(t => t["TABLE_NAME"]).ToDictionary(t => t.Key as string, t => t.ToList());
                            foreach (var table in tables)
                            {
                                try
                                {
                                    ...
                                    foreach (var row in table.Value)
                                    {
                                        ...
                                        tbl.Type = type != null && type.Equals("VIEW", StringComparison.CurrentCultureIgnoreCase) ? TableType.View : TableType.Default;
                                        var col = new Column();
                                        ...
                                        col.Nullable = ((row["IS_NULLABLE"] as string) ?? string.Empty).Equals("YES", StringComparison.OrdinalIgnoreCase);
                                        col.Owner = row["TABLE_SCHEMA"] as string;
                                        col.PrimaryKey = ((row["PRIMARY_KEY"] as string) ?? string.Empty).Equals("YES", StringComparison.OrdinalIgnoreCase);
                                        ...
                                        {
                                            col.ForeignKeys = new List<ForeignKey> { new ForeignKey { ForeignColumn = foreignColumn, ForeignTable = foreignTable } };
                                        }
                                        tbl.Columns.Add(col);
                                    }
                                    tmp.Add(tbl);
        ...
        protected string GetCmd()
        {
            ...
            using (var cmdStream = GetType().Assembly.GetManifestResourceStream("xCoder.Logic.Resource.MSSQL.Columns.sql"))
                if (cmdStream != null)
