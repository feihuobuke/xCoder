...
using System.Linq;
using System.Xml;
using SF.xCoder.DB2Project.Comm;
    ...
    public class HBMBuilder : AbsBuilder
    {
        ...
        protected string[] HBMFileGenerate()
        {
            ...
            foreach (var table in DataBase.Tables)
            {
                ...
                tableAttribute.Value = string.Format(string.IsNullOrEmpty(table.Owner) ? "{1}" : "{0}.{1}", table.Owner,
                                                     table.Name);
                ...
                var parents =
                    table.Parents.Where(
                        t => !string.IsNullOrEmpty(t.ColumnRelated) && !string.IsNullOrEmpty(t.TableRelated)).ToList();
                    ...
                    var schema = new XmlNodeSchema
                                     {
                                         ...
                                         Properties =
                                             new NameValueCollection
                                                 {
                                                     {"name", parent.ClassName ?? parent.TableRelated},
                                                     {"column", parent.Column},
                                                     {"cascade", "none"},
                                                     {"unique", "true"},
                                                     {"class", AssemblyTypeFormat(parent)},
                                                 }
                                     };
                    classNode.Write(schema);
                }
                classNode.Attributes.Remove(
                    classNode.Attributes.OfType<XmlAttribute>().FirstOrDefault(
                        t => t.Name.Equals("xmlns", StringComparison.OrdinalIgnoreCase)));
                var childs =
                    table.Childs.Where(
                        t => !string.IsNullOrEmpty(t.ColumnRelated) && !string.IsNullOrEmpty(t.TableRelated)).ToList();
                                                ...
                                                new XmlNodeSchema
                                                    {
                                                        Name = "key",
                                                        Properties =
                                                            new NameValueCollection {{"column", child.ColumnRelated}}
                                                    },
                                                new XmlNodeSchema
                                                    {
                                                        Name = "one-to-many",
                                                        Properties =
                                                            new NameValueCollection
                                                                {{"class", AssemblyTypeFormat(child)}}
                                                    }
                                            };
                    classNode.Write(schema);
                }
                ...
                var fileName = Path.Combine(Directory.FullName, string.Format("{0}.hbm.xml", table.Name));
                xml.Save(fileName);
                tmp.Add(fileName);
            }
            return tmp.ToArray();
