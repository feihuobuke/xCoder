...
namespace SF.xCoder.DB2Project.Data.Reader
{
    internal class SqlCeReader : AbsReader
    {
                    ...
                    using (var reader = Command.ExecuteReader())
                    {
                        try
                        {
                            while (reader.Read())
                            {
                                ...
                                col.Nullable = ((reader["IS_NULLABLE"] as string) ?? string.Empty).Equals("YES",
                                                                                                          StringComparison
                                                                                                              .
                                                                                                              OrdinalIgnoreCase);
                                //col.Owner = row["TABLE_SCHEMA"] as string;
                                col.PrimaryKey = ((reader["PRIMARY_KEY"] as string) ?? string.Empty).Equals("YES",
                                                                                                            StringComparison
                                                                                                                .
                                                                                                                OrdinalIgnoreCase);
                                ...
                                {
                                    var parent = new TableRelation
                                                     {
                                                         Column = col.Name,
                                                         ColumnRelated = foreignColumn,
                                                         TableRelated = foreignTable
                                                     };
        ...
        protected string GetCmd()
        {
            ...
            using (
                var cmdStream =
                    GetType().Assembly.GetManifestResourceStream("SF.xCoder.DB2Project.Resource.SQLCE.Columns.sql"))
                if (cmdStream != null)
